                ...
                gint        n_fds)
{
   ...
   * it was originally set. But that opens a small race window where
   * the fd could be inherited into a child process if another
   * thread spawns something at the same time. It is avoidable by
   * using the same strategy that we already require to handle
   * source_fds[i] != target_fds[i], so we might as well always
   * dupfd_cloexec here.
   */
